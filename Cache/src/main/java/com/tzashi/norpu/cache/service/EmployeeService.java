package com.tzashi.norpu.cache.service;
...
public class EmployeeService {
     ...
     *
     *      key：缓存数据使用的key；可以用它来指定。默认是使用方法参数的值，键值对： 1-方法的返回值
     *              支持SpEL： #id 取出参数id的值   #a0  #p0  #root.args[0]
     *              getEmp[2]
     *
     ...
     *      sync：是否使用异步模式：不支持 unless，方法未执行完就进行缓存
     */
     ...
     *             @Cacheable的key是不能用#result，方法运行前放入数据
